...
onMounted(() => {
    ...
    .then(() => {
        ...
        console.log(`Route Query ID: ${query_id}`)
        console.log(`QueryStore has: ${queryStore.queriesAsMap.has(query_id)}`)
        console.log(`QueryStore KEYS: ${typeof Array.from(queryStore.queriesAsMap.keys())[0]}`)
        query.value = queryStore.queriesAsMap.get(query_id)
        query_editor = createEditor(monaco_container.value, query.value?.query, true)
        console.log(`Ref Query ID: ${query.value?.queryId}`)
        console.log(`All Queries: ${queryStore.queriesAsList.map(q => typeof q.queryId)}`)
    })
    .then(() => {
        updateMatchList()
        getParsed()
...
function getParsed() {
    ...
    console.log(query.value?.queryId)
    options.headers.set('Content-Type', 'application/json')
...
function getPlan() {
    ...
    options.headers.set('Content-Type', 'application/json')
    fetch(`http://${backendIp}/query/plan`, options)
    // .then((res) => res.json())
    .then((res) => res.json() )
    .then((data) => {
...
function getLatest() {
    if(max_visible.value === 0) {
        socket.value?.emit("read_stream", {"query_id": query.value?.queryId, "latest": latest.value, "earliest": latest.value - 19})
    } else {
...
function getLater() {
    ...
    if(max_visible.value !== min_visible.value) {
        if((max_visible.value + 20) < latest.value ) {
            socket.value?.emit("read_stream", {"query_id": query.value?.queryId, "latest": max_visible.value + 20, "earliest": max_visible.value + 1})
        } else  { 
            // )max_visible.value + 20) >= latest.value
            socket.value?.emit("read_stream", {"query_id": query.value?.queryId, "latest": latest.value, "earliest": max_visible.value + 1})
        }
    } else {
        if((latest.value - earliest.value) > 20) {
            ...
            socket.value?.emit("read_stream", {"query_id": query.value?.queryId, "latest": latest.value, "earliest": earliest.value})
        }
    }
...
function getEarlier() {
    if(min_visible.value !== max_visible.value) {
        if((min_visible.value - 20) > earliest.value ) {
            socket.value?.emit("read_stream", {"query_id": query.value?.queryId, "latest": min_visible.value-1, "earliest": min_visible.value - 20})
        } else {
            // (min_visible.value - 20) <= earliest.value)
            socket.value?.emit("read_stream", {"query_id": query.value?.queryId, "latest": min_visible.value-1, "earliest": earliest.value})
        }
    } else {
        if((latest.value - earliest.value) > 20) {
...
<template>
    ...
    <div class="row">
        <div class="col">
            <div :class="{'d-none': !show_query}" class="my-2">
                <p>Submitted Query:</p>
                <div id="container" ref="monaco_container" style="height: 30vh;">
                </div>    
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col">
            ...
            <button @click="show_fields = !show_fields" class="mx-2 btn btn-outline-dark">
                <i class="bi" :class="{ 'bi-eye': show_fields, 'bi-eye-slash': !show_fields}"></i>
                Toggle Field Names
            </button>
            <button @click="show_timestamps = !show_timestamps" class="mx-2 btn btn-outline-dark">
                <i class="bi" :class="{ 'bi-eye': show_timestamps, 'bi-eye-slash': !show_timestamps}"></i>
                Toggle Timestamps
            </button>
            ...
            <button @click="show_parsed = !show_parsed" class="mx-2 btn btn-outline-dark">
                <i class="bi bi-map"></i>
                Show Parsed
            </button>
            <button @click="show_plan = !show_plan" class="mx-2 btn btn-outline-dark">
                <i class="bi bi-map"></i>
                Show Plan
            </button>
    ...
    <div class="row">
        <div class="col">
            ...
            <EventTypeInput v-if="query?.queryId" :event_types="query_event_types" :query_id="query?.queryId"></EventTypeInput>
        </div>
    </div>
